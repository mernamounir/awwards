...
(function (factory) {
	...
	$.fn.jR3DCarousel = function(options){
		var _defaults = {
				...
				animationCurve: 'ease-in-out',			/* ease | ease-in | ease-out | ease-in-out | linear | bezier */
		...
		(function setup(){
			...
			function createjR3DCarousel(){
				...
				if(_settings.slides.length){					
					for(var i = 0;  i < _settings.slides.length; i++){
						var slide = $( `<a href ="`+ _settings.slides[i].urlImage+`"target="_blank" <div class='jR3DCarouselSlide py-5' data-index=`+i+` /> </a>` ).append( `<img src=`+_settings.slides[i].src+` alt=`+_settings.slides[i].alt+`/>  <h3 class="pb-3">`+_settings.slides[i].titl+`</h3>
                            <p class="text-muted ">`+_settings.slides[i].des+`</p>`);
						if(_settings.animation.indexOf('slide')!=-1){
			...
			function _createNavigation(){
				var _navigation = $('<div class=navigation />').css({ position: 'absolute', bottom:-130, right:0  });
				for(var i = 0;  i < _noOfSlides; i++){
					_navigation.append('<div class=nav1 ></div>');
				}
				if(_settings.navigation == 'circles'){
					_navigation.find('.nav1').css({ borderRadius: '12px' });
				}
				_navigation.find('.nav1').css({ display: 'inline-block', margin: '5px', cursor: 'pointer', backgroundColor: 'rgba(255,255,255,0.5)', width: '12px', height: '12px', transition: 'all '+_settings.animationDuration+'ms ease' })
										.first().css({ backgroundColor:'#ffb44b'  });
				_jR3DCarouselDiv.after(_navigation);
				
				 /* event handler */
				_container.on('click', '.nav1', function(){
		...
		function _slideCarouseld(){
			_container.find('.nav1').css({  backgroundColor: 'rgba(255, 255, 255, 0.77)' })
			   .eq(_targetSlideIndex % _noOfSlides).css({ backgroundColor: '#ffb44b' });
